 ...
 * against geometry.json's parallel coordinate arrays.
 *
...
export function buildRenderableSegments(
  ...
  return timeslot.segment_idx.map((segmentIdx, i) => ({
    segmentIdx,
    startLat: geometry.start_lat[segmentIdx],
    startLon: geometry.start_lon[segmentIdx],
    endLat: geometry.end_lat[segmentIdx],
    endLon: geometry.end_lon[segmentIdx],
